#!/usr/bin/env python3
"""Create multilingual embeddings for the card document corpus."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Embed the retrieval-ready card documents with a multilingual model."
    )
    parser.add_argument(
        "--input",
        default="data/cards_documents.jsonl",
        help="Path to the document JSONL file",
    )
    parser.add_argument(
        "--output-dir",
...
def main() -> int:
    ...
    if not texts:
        raise SystemExit("No documents found in the input file.")

    output_dir.mkdir(parents=True, exist_ok=True)
    embeddings_path = output_dir / "card_embeddings.npy"
    metadata_path = output_dir / "card_embeddings_metadata.jsonl"
